...
fn non_finite_float_literals_follow_numeric_semantics() {
    check(
        "(expect ($ \"%t:%s\" NaN NaN) \"float:NaN\") (expect ($ \"%t:%s\" Inf Inf) \"float:Inf\") (expect ($ \"%t:%s\" -Inf -Inf) \"float:-Inf\") (expect (eq NaN NaN) f) (expect (eq Inf Inf) t) (expect (eq -Inf -Inf) t) (expect (eq 0.0 -0.0) t) (expect (eq 5 5.0) t) (expect (lt 1.0 Inf) t) (expect (lt -Inf 1.0) t) (expect (lt NaN 1.0) f) (expect (gt NaN 1.0) f)",
        "t",
    );
}
